...
def draw_fingertap(rgb_image, detection_result):
    """
    Draws landmarks on hand model-detected outputs for fore-finger to thumb tapping as 
    part of UPDRS-III assessment.
    ...
    if not detection_result.hand_landmarks or not detection_result.handedness:
        print("Either hand landmarks or handedness not detected.")
...
# Method for extracting points for fingertaps
def extract_points_for_fingertap(result: HandLandmarkerResult):
    """
    Extracts points for fingertap detection from MediaPipe results.
    Args:
        result: The HandLandmarkerResult object for the frame.
    Returns:
        A tuple containing the normalized Y coordinates of the index finger tip and thumb tip.
    """
    current_index_y = np.nan # Default to NaN if not found
    current_thumb_y = np.nan # Default to NaN if not found
    ...
    if result.hand_landmarks and result.handedness:
        if len(result.handedness) == len(result.hand_landmarks):
            for handedness_list, hand_landmarks in zip(result.handedness, result.hand_landmarks):
                ...
                # For now, proceeding with assumption that right hand is the one to process
                hand_label = handedness_list[0].category_name
                if hand_label.lower() == 'right':

                    # Check if landmark indices are valid
                    if INDEX_FINGER_TIP_INDEX < len(hand_landmarks):
                        # Store normalized Y (0.0 at top, 1.0 at bottom)
                        current_index_y = hand_landmarks[INDEX_FINGER_TIP_INDEX].y
                    if THUMB_TIP_INDEX < len(hand_landmarks):
...
hand_present_list = []
# TODO: Add exif and blur/motion values to this list which is converted to a df, then csv
...
with HandLandmarker.create_from_options(hand_options) as handmarker,\
    ...
    while cap.isOpened() and frame_index < 300:  # Limit to 1000 frames for testing
        ...
        # Format frame for processing with Landmarkers
        ...
        # --- Making annotated frame ---
        # Process the result (e.g., draw landmarks or save results)
        de_id_frame = blur_face_pose(frame_rgb, pose_result)# now using the pose face-markers as the way to 
        out_frame = draw_fingertap(de_id_frame, result)#draw hand outputs on image
        # out_frame = draw_landmarks_on_image(
        #     out_frame, pose_result,
        #     exclude_landmarks=['ALL_HANDS_POSE'], # Use keywords for exclusion
        #     circle_radius=2
        # )
        ...
        # --- Determine if hand is present, get landmarks ---
        hand_present = len(result.hand_landmarks)
        hand_present_list.append(hand_present)

        if hand_present > 0:
            # Extract points for fingertap detection
            current_index_y, current_thumb_y = extract_points_for_fingertap(result)
            right_index_tip_y_coords.append(current_index_y)
            right_thumb_tip_y_coords.append(current_thumb_y)
        else:
            right_index_tip_y_coords.append(np.nan)
            right_thumb_tip_y_coords.append(np.nan)
...
ax.plot(frame_indexes, right_index_tip_y_coords, label='Right Index Tip Y', color='red', marker='.', linestyle='-', markersize=4)
ax.plot(frame_indexes, right_thumb_tip_y_coords, label='Right Thumb Tip Y', color='blue', marker='.', linestyle='-', markersize=4)
...
output_plot_filename = os.path.join('OutputPlots','keypoint_plot'+video_name_tag+'.jpg')
makedirs('OutputPlots', exist_ok=True)
try:
